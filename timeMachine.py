...
MAIN_DIRECTORY = "/home/scz/timeline_python"
# Backup Directory
...
# ArgParse is the one setting up all the CLI commands we are to give
# And parse them into a dictionary so as we can access
...
# Here Setting up app name and DEBUG Level depicting logs are provided
# when we want to solve an issue or information logs
...
# List the files in YAML format after taking out the files from config dat.
# This function will be used in add, remove, update functionality
...
# This function is the final step in creating the copy directory according to algorithm[3].
# These functions are used for code cleanup
...
def remove_file(param_config, remove):
    try:
        ...
        sys.exit(1)
    pass
# This function doing addition of desired path into the config.dat file
# at the same time in backup folder adding the directory
# file as the first update
...
def add_file(config, add):
    try:
        ...
        sys.exit(1)
    pass
# This function will be the update function allowing us to check
# add the files to backup folder by creating a directory for new files
...
def check_files_and_copy(configData, paramDir):
    ...
    # configData is data from our config.dat file in yaml format.
    # This is checked regularly for any changes later in cron
    for file in filesList:
        paths.append(file['file'])
    if not os.path.exists(targetDirectory):
        # Here We will avoid an error and create a directory to backup
        # if directory is not present ie first time we run the program
    ...
    for path in paths:
        ...
        if os.path.exists(path):
            ...
            # print("output", os.path.getmtime(output_directory))
            # function creating path according to algorithm
        ...
        if os.path.getmtime(path) > os.path.getmtime(output_directory):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if param_remove is not None:
            remove_file(CONFIG_FILE, param_remove)
            print("\nFILE REMOVED")
